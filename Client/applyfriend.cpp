    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    // 隐藏对话框标题栏
    this->setWindowTitle("申请添加好友");
    //setWindowFlags(windowFlags() | Qt::FramelessWindowHint);
    this->setObjectName("ApplyFriend");
    this->setModal(true);
    ui->lb_ed->setPlaceholderText("搜索、添加标签");
    ui->remark_name_ed->setPlaceholderText("燃烧的胸毛");
    ...
    //链接输入标签回车事件
    connect(ui->lb_ed, &SearchBoxEdit::returnPressed, this, &ApplyFriend::SlotLabelEnter);
    connect(ui->lb_ed, &SearchBoxEdit::textChanged, this, &ApplyFriend::SlotLabelTextChange);
    connect(ui->lb_ed, &SearchBoxEdit::editingFinished, this, &ApplyFriend::SlotLabelEditFinished);
...
void ApplyFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        connect(lb, &ClickableLabel::clicked, this, &ApplyFriend::SlotChangeFriendLabelByTip);
...
void ApplyFriend::AddTipLbs(ClickableLabel* lb, QPoint cur_point, QPoint& next_point, int text_width, int text_height)
{
    lb->move(cur_point);
    lb->show();
...
void ApplyFriend::SetSearchInfo(std::shared_ptr<UserInfo> user_info)
{
    user_info_ = user_info;
    auto bakname = user_info_->name_;
    ui->request_message_ed->setPlaceholderText("好友申请信息");
    ui->remark_name_ed->setPlaceholderText(bakname);
}
...
void ApplyFriend::ShowMoreLabel()
{
    ...
    for(auto & added_key : _add_label_keys){
        ...
        int textWidth = fontMetrics.horizontalAdvance(added_lb->text()); // 获取文本的宽度
...
void ApplyFriend::SlotApplySure()
{
    ...
    jsonObj["peer_uid"] = user_info_->uid_;
    jsonObj["request_message"] = ui->request_message_ed->text();
    jsonObj["remark_name"] = ui->remark_name_ed->text();
